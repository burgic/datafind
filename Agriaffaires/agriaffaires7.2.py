...
def create_session():
    ...
    session.headers.update({
        ...
        'Referer': 'https://www.agriaffaires.co.uk/',
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"Retrying with a new proxy in {wait_time} seconds...")
...
def main():
    ...
    listings = fetch_listing_urls(main_url, random.choice(sessions))

    # Step 2: Fetch detailed data for each listing
    detailed_listings = []
    for listing in listings:
        session = get_random_session()
        detailed_info = fetch_listing_details(listing['URL'], random.choice(sessions))
        if detailed_info:
