...
def create_session():
    ...
    headers = {
        ...
        'Referer': 'https://www.farmmachinerylocator.co.uk/',
        'DNT': '1',
    ...
    while max_retries > 0:
        ...
        try:
            ...
            if len(listing_blocks) == 0:
                print("Searching for any div with 'listing' in class name:")
                any_listings = soup.find_all('div', class_=lambda x: x and 'listing' in x)
                print(f"Found {len(any_listings)} divs with 'listing' in class name")
                if any_listings:
                    print("Classes found:", [div.get('class') for div in any_listings[:5]])
